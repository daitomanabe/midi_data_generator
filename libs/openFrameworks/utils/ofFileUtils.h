...
ofBuffer ofBufferFromFile(const std::__fs::filesystem::path & path, bool binary=true);
...
/// split at endline characters automatically
bool ofBufferToFile(const std::__fs::filesystem::path & path, const ofBuffer& buffer, bool binary=true);
...
public:
    ...
    static std::string getFileExt(const std::__fs::filesystem::path& filename);
	
	/// Remove extension from a filename, ie. "duck.jpg" ->"duck".
	///
	/// \param filename file path
	/// \returns filename without extension
    static std::string removeExt(const std::__fs::filesystem::path& filename);
	
	/// Prepend path with a slash, ie. "images" -> "/images".
	///
	/// \param path file or directory path
	/// \returns slah + path
    static std::string addLeadingSlash(const std::__fs::filesystem::path& path);
	
	/// Append path with a slash, ie. "images" -> "images/".
	///
	/// \param path directory path
	/// \returns path + slash
    static std::string addTrailingSlash(const std::__fs::filesystem::path& path);
    ...
    static std::string removeTrailingSlash(const std::__fs::filesystem::path& path);
    ...
    static std::string getPathForDirectory(const std::__fs::filesystem::path& path);
    ...
    static std::string getAbsolutePath(const std::__fs::filesystem::path& path, bool bRelativeToData = true);
    ...
    static bool isAbsolute(const std::__fs::filesystem::path& path);
	...
	/// "../../"
	/// \returns filename
    static std::string getFileName(const std::__fs::filesystem::path& filePath, bool bRelativeToData = true);
	...
	/// \param filePath file path
	/// \returns basename
    static std::string getBaseName(const std::__fs::filesystem::path& filePath);
    ...
    static std::string getEnclosingDirectory(const std::__fs::filesystem::path& filePath, bool bRelativeToData = true);
    ...
    static bool createEnclosingDirectory(const std::__fs::filesystem::path& filePath, bool bRelativeToData = true, bool bRecursive = true);
	...
	/// \returns joined path
    static std::string join(const std::__fs::filesystem::path& path1, const std::__fs::filesystem::path& path2);
    ...
    static std::string makeRelative(const std::__fs::filesystem::path & from, const std::__fs::filesystem::path & to);
};
...
public:
	...
	ofFile(const std::__fs::filesystem::path & path, Mode mode=ReadOnly, bool binary=true);
	...
	bool open(const std::__fs::filesystem::path & path, Mode mode=ReadOnly, bool binary=true);
	...
	/// split at endline characters automatically
	/// \returns true if the path was opened
	bool openFromCWD(const std::__fs::filesystem::path & path, Mode mode=ReadOnly, bool binary=true);
	...
	/// \param path file path
	/// \returns true if the file was created
	bool create(const std::__fs::filesystem::path & path);
	...
	bool copyTo(const std::__fs::filesystem::path& path, bool bRelativeToData = true, bool overwrite = false) const;
	...
	/// directory at the new path
	/// \returns true if the copy was successful
	bool moveTo(const std::__fs::filesystem::path& path, bool bRelativeToData = true, bool overwrite = false);
	...
	/// directory at the new path
	/// \returns true if the copy was successful
	bool renameTo(const std::__fs::filesystem::path& path, bool bRelativeToData = true, bool overwrite = false);
	...
	operator std::__fs::filesystem::path(){
		return myFile;
	}

	operator const std::__fs::filesystem::path() const{
		return myFile;
	}
	...
	/// directory at the new path
	/// \returns true if the copy was successful
	static bool copyFromTo(const std::__fs::filesystem::path& pathSrc, const std::__fs::filesystem::path& pathDst, bool bRelativeToData = true,  bool overwrite = false);
	...
	static bool moveFromTo(const std::__fs::filesystem::path& pathSrc, const std::__fs::filesystem::path& pathDst, bool bRelativeToData = true, bool overwrite = false);
	...
	static bool doesFileExist(const std::__fs::filesystem::path& fPath,  bool bRelativeToData = true);
	...
	/// "../../"
	/// \returns true if the path was removed successfully
	static bool removeFile(const std::__fs::filesystem::path& path, bool bRelativeToData = true);

private:
	bool isWriteMode();
	bool openStream(Mode _mode, bool binary);
	void copyFrom(const ofFile & mom);
	std::__fs::filesystem::path myFile;
	Mode mode;
	bool binary;
};
...
public:
	...
	///
	/// \param path directory path
	ofDirectory(const std::__fs::filesystem::path & path);

	/// Open a directory path, clears the current file list.
	///
	/// \param path directory path
	void open(const std::__fs::filesystem::path & path);
	...
	///
	/// \param path directory path
	void openFromCWD(const std::__fs::filesystem::path & path);
	...
	/// directory at the new path
	/// \returns true if the copy was successful
	bool copyTo(const std::__fs::filesystem::path& path, bool bRelativeToData = true, bool overwrite = false);
	...
	static bool createDirectory(const std::__fs::filesystem::path& dirPath, bool bRelativeToData = true, bool recursive = false);
	...
	/// directories
	static bool isDirectoryEmpty(const std::__fs::filesystem::path& dirPath, bool bRelativeToData = true );
	...
	static bool doesDirectoryExist(const std::__fs::filesystem::path& dirPath, bool bRelativeToData = true);
	...
	/// are *not* in the data directory
	/// \returns true if the path was removed successfully
	static bool removeDirectory(const std::__fs::filesystem::path& path, bool deleteIfNotEmpty,  bool bRelativeToData = true);
...
private:
	std::__fs::filesystem::path myDir;
